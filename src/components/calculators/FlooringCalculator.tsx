  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3">
        <label className="space-y-1"><span className="text-xs font-medium">Room Length (ft)</span><input placeholder="e.g. 5.5" type="number" value={length} onChange={(e) => setLength(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-secondary/50 border border-border focus:border-primary outline-none text-sm" min="0" /></label>
        <label className="space-y-1"><span className="text-xs font-medium">Room Width (ft)</span><input placeholder="e.g. 5.5" type="number" value={width} onChange={(e) => setWidth(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-secondary/50 border border-border focus:border-primary outline-none text-sm" min="0" /></label>
        <label className="space-y-1"><span className="text-xs font-medium">Tile Size (inches)</span><input placeholder="e.g. 5000" type="number" value={tileSize} onChange={(e) => setTileSize(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-secondary/50 border border-border focus:border-primary outline-none text-sm" min="1" /></label>
        <label className="space-y-1"><span className="text-xs font-medium">Waste % (cuts)</span><input placeholder="e.g. 5.5" type="number" value={waste} onChange={(e) => setWaste(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-secondary/50 border border-border focus:border-primary outline-none text-sm" min="0" max="50" /></label>
      </div>
