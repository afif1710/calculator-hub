  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        <label className="space-y-1"><span className="text-xs font-medium">{mode === 'hourly' ? 'Hourly Rate ($)' : 'Annual Salary ($)'}</span><input placeholder="e.g. 5.5" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-secondary/50 border border-border focus:border-primary outline-none text-sm" min="0" /></label>
        <label className="space-y-1"><span className="text-xs font-medium">Hours/Week</span><input placeholder="e.g. 5.5" type="number" value={hoursPerWeek} onChange={(e) => setHoursPerWeek(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-secondary/50 border border-border focus:border-primary outline-none text-sm" min="1" /></label>
        <label className="space-y-1"><span className="text-xs font-medium">Weeks/Year</span><input placeholder="e.g. 5.5" type="number" value={weeksPerYear} onChange={(e) => setWeeksPerYear(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-secondary/50 border border-border focus:border-primary outline-none text-sm" min="1" max="52" /></label>
      </div>
